...
var db, accounts;
MongoClient.connect(process.env.DB_URL, { useNewUrlParser: true,uri_decode_auth: true  }, function(e, client) {
	if (e){
		console.log(e);
	}	else{
	...
	// index fields 'user' & 'email' for faster new account validation //
		...
		courses.createIndex({courseName: 1, courseList: 1});
...
exports.getUserCount = function(collection_table,o,callback){
	if(!o){
		o = null;
	}
	if(collection_table){
		db.collection(collection_table).countDocuments(o,function(err,count){
			if(err)
			{
				throw err;
			}
...
exports.addNewCourse = function(collection_table, newCourse, callback)
{
	db.collection(collection_table).findOne(newCourse, function(e, o) {	
		if(e){
			throw e;
		}
		if (o){
			callback('course already exist');
		}	else{
				newCourse.date = moment().format('MMMM Do YYYY, h:mm:ss a');
				db.collection(collection_table).insertOne(newCourse, function(){callback('successfully inserted')});
			}
		});
...
exports.getAllRecords = function(collection_table ,callback)
{
	console.log(collection_table,callback);
	db.collection(collection_table).find().toArray(
		function(e, res) {
		if (e) callback(e)
